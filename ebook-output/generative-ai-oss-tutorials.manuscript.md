...
### 1.1.8 実ソースコード（言語別に記載）

#### 1.1.8.1 Python: 01_basic-chain.py
...
#### 1.1.8.3 Python: 03_memory-persistence.py
...
#### 1.1.8.4 JavaScript: 01_basic-chain.js
...
#### 1.1.8.5 JavaScript: 02_tool-use.js
...
#### 1.1.8.6 JavaScript: 03_memory-persistence.js
...
#### 1.2.8.1 Python: 01_basic-workflow.py
...
#### 1.2.8.2 Python: 01_basic-workflow-llm.py
...
#### 1.2.8.3 Python: 02_state-management.py
...
#### 1.2.8.4 JavaScript: 01_basic-workflow.js
...
### 1.3.7 実ソースコード（言語別に記載）
#### 1.3.7.1 Python: requirements.txt
...
#### 1.3.7.2 Python: 01_two-agents-chat.py（GroupChat/GroupChatManager版）
...
#### 1.4.3.1 タスク分割・役割分担の設計指針
- ゴールを明確化し、責務ごとにAgent/Taskを分割
- レビュー・改善・検証など多段階化も容易

#### 1.4.3.2 「繰り返し制御」の可否と実現方法
...
#### 1.4.4.2 Python: requirements.txt
...
#### 1.4.5.3 2. 依存パッケージのインストール
```bash
uv pip install -r requirements.txt
```

#### 1.4.5.4 3. 文字化け・cp932エラー対策（Windows）
...
#### 1.4.5.5 4. CrewAIサンプルの実行と証拠ファイル生成
```bash
...
### 2.1.9 公式情報の参照・引用について
本教材の内容は公式サイト等の一次情報
（[公式ドキュメント](https://docs.llamaindex.ai/)、
[FAQ](https://docs.llamaindex.ai/en/stable/getting_started/faq/)等）
を参照し、2026年5月時点で整理しています。

---

### 2.1.10 実ソースコード（言語別）

#### 2.1.10.1 Python: 01_basic-indexing.py
...
##### 2.1.10.1.1 実行結果例（01_basic-indexing.py）
...
#### 2.1.10.3 Python: 03_advanced-retrieval.py
...
##### 2.1.10.3.1 実行結果例（03_advanced-retrieval.py）
...
- Python 3.12系と仮想環境（uv/venv）の利用を推奨
...
#### 2.2.7.2 Python: requirements.txt（Haystack 2.x対応例）
...
#### 2.2.7.3 Python: 01_basic-pipeline.py（Haystack 2.x対応）
...
##### 2.2.7.3.1 実行結果例（01_basic-pipeline.py）
...
### 2.2.10 公式情報の参照・引用について
本教材の内容は公式サイト等の一次情報
（[公式ドキュメント](https://docs.haystack.deepset.ai/)、
[Migration Guide](https://docs.haystack.deepset.ai/docs/migration-guide)等）
を参照し、2026年5月時点で整理しています。
...
1. Haystack 入門を使う想定ユースケースを1つ定義し、入力・出力の例を記録してください。
2. 最小構成で動かし、デフォルトから設定を1つ変えて挙動の差分を確認してください。
3. Haystack 入門を使わない場合の代替手段と比較し、選ぶ基準をまとめてください。
...
1. Haystack 入門の主な役割を1文で説明してください。
2. Haystack 入門を導入する際の最大のメリットと注意点は何ですか？
3. Haystack 入門が向かないユースケースとして、どのようなケースが考えられますか？
...
#### 2.3.3.3 できないこと・制約事項（Limitations）
- 高度なUI機能は非搭載（外部実装が必要）
- 大規模分散処理や商用DB連携は限定的
...
#### 2.3.9.1 Python: requirements.txt
- 役割: txtaiデモの依存関係定義
- 入力: なし
- 出力: インストール対象リスト
- 実行: `uv pip install -r requirements.txt`
...
#### 2.3.9.2 Python: 01_basic-search.py
- 役割: 最小のセマンティック検索
- 入力: 文書配列と検索クエリ
- 出力: 上位検索結果
...
#### 2.5.6.1 Python: requirements.txt（最小構成例）
...
#### 3.1.6.2 Setup: 01_setup-guide.md
...
#### 3.1.6.3 Setup: 02_cli-examples.sh
...
#### 3.3.6.2 Setup: 01_setup-guide.md
...
#### 3.3.6.3 Setup: 02_request-examples.ps1
...
##### 4.1.10.1.1 0. 作業ディレクトリ準備（PowerShell）
...
##### 4.1.10.1.2 1. docker-compose.yml を作成
...
##### 4.1.10.1.4 3. 使うモデルを Ollama に取得
...
##### 4.1.10.1.5 4. Open WebUI 初期アクセス
...
##### 4.1.10.1.8 6. 基本機能の完了判定（最低ライン）

- UI からチャット送信できる
- ローカルモデルから応答が返る
- 会話履歴がサイドバーに保存される

##### 4.1.10.1.9 7. 停止・再開（検証用）
...
##### 4.2.10.1.2 1. 公式リポジトリを取得（同梱 Compose を利用）
...
##### 4.2.10.1.7 5.1 Workflow Builder（ドラッグ&ドロップ）
...
##### 4.3.10.1.6 5. Provider 設定とテスト実行
...
##### 4.3.10.1.7 5.1 Builder モード選択の確認
...
##### 4.4.13.1.1 0. 作業ディレクトリ準備（PowerShell）
...
##### 4.4.13.1.2 1. docker-compose.yml と設定ファイルを準備
...
##### 4.4.13.1.7 5.1 Tool Call / MCP 価値確認
...
##### 4.5.11.1.1 0. 作業ディレクトリ準備（PowerShell）
...
##### 4.5.11.1.3 2. 環境変数を設定（公式 Quickstart 準拠）
...
##### 4.5.11.1.8 6. 基本機能の完了判定（最低ライン）

- UI が表示される
- API キー設定で応答が返る
- エラー表示時に原因を特定できる

##### 4.5.11.1.9 7. 停止・再開（検証用）
...
##### 4.7.10.1.1 0. 作業ディレクトリ準備（PowerShell）
...
##### 4.7.10.1.2 1. docker-compose.yml を作成
...
##### 4.7.10.1.8 6. 基本機能の完了判定（最低ライン）

- ワークスペース作成が完了する
- Provider 設定で応答が返る
- 1ファイル以上を取り込んで回答できる

##### 4.7.10.1.9 7. 停止・再開（検証用）
...
#### 5.1.10.1 JSON: examples/promptfoo/package.json
...
#### 5.1.10.2 YAML: examples/promptfoo/00_promptfooconfig.yaml
...
##### 5.1.10.2.1 assert type の意味と代表的な値一覧

promptfoo の `assert` には複数の type があり、出力の自動判定に使います。

##### 5.1.10.2.2 代表的な type
...
##### 5.1.10.2.3 その他の主な type（2024年5月時点）
...
#### 5.2.10.1 01_basic-ragas-eval.py
...
#### 5.4.10.1 01_basic-validation.py
...
#### 5.4.10.2 02_output-format-check.py
...
### 6.1.7 実ソースコード（言語別に記載）
#### 6.1.7.1 00_requirements.txt
...
#### 6.3.7.2 03_comfyui-python/00_requirements.txt

```txt
requests==2.32.3
```

#### 6.3.7.3 03_comfyui-python/01_queue-prompt.py
...
#### 6.4.7.2 04_automatic1111-python/00_requirements.txt

```txt
requests==2.32.3
```

#### 6.4.7.3 04_automatic1111-python/01_txt2img.py
...
### 7.1.9 実ソースコード（言語別に記載）
#### 7.1.9.1 JavaScript: examples/vega-lite/01_generate-spec.js
...
#### 7.1.9.2 JSON: examples/vega-lite/02_chart-spec.json
...
#### 7.2.9.1 JSON: examples/echarts/package.json
...
#### 7.2.9.2 JavaScript: examples/echarts/01_option-template.js
...
#### 7.2.9.3 JSON: examples/echarts/02_echarts-option.json
...
#### 7.2.9.5 Vega-LiteとEChartsのJSON比較（理解を深める観点）
...
## 8.3 MCP 実践編: stock-analyzer バックエンド連携
...
### 8.3.7 実ソースコード（言語別に記載）
#### 8.3.7.1 03_backend-integration-python/00_requirements.txt
...
#### 8.3.7.2 03_backend-integration-python/01_load-config.py
...
#### 8.3.7.3 03_backend-integration-python/02_agent-request-example.py
