# KDP Registration Package: 生成AI活用OSS 実践ガイド

- Generated: 2026-05-24 22:05:39
- Project: generative-ai-oss-tutorials
- Metadata: C:\Dev\tutorials\generative-ai-oss-tutorials\.\.github\skills-config\ebook-build\generative-ai-oss-tutorials.metadata.yaml
- KDP Metadata: C:\Dev\tutorials\generative-ai-oss-tutorials\.\.github\skills-config\ebook-build\generative-ai-oss-tutorials.kdp.yaml

## 1. Bibliographic Information

| Field | Value |
|---|---|
| Title | 生成AI活用OSS 実践ガイド |
| Subtitle | オープンソースで学ぶ生成AIスタックの全体像 |
| Author | 杜 文吉 |
| Language | ja-JP |
| Publisher | Duwenji Lab |
| Rights | Copyright (c) 杜 文吉 |
| Identifier | generative-ai-oss-tutorials |
| Publication Date | 2026-05-10 |
| ISBN | Typically not required for Kindle only |

## 2. Product Description

> 生成AI関連OSSを比較しながら学べる実践教材。カテゴリ別に最小実行手順と選定観点を整理し、 開発・検証・運用に必要な基礎を短時間で習得できます。

## 3. Keywords (up to 7)

- 生成AI
- OSS
- RAG
- LLM
- 開発効率
- 評価
- チュートリアル

## 4. Categories (up to 3)

- Computers / Software Development
- Computers / Programming

## 5. Pricing and Distribution

| Field | Value |
|---|---|
| List Price | 1490 JPY |
| Royalty | 70% |
| Territories | Worldwide |
| KDP Select | No |

## 6. Upload Files

- EPUB: C:\Dev\tutorials\generative-ai-oss-tutorials\.\ebook-output\generative-ai-oss-tutorials.epub
- PDF: C:\Dev\tutorials\generative-ai-oss-tutorials\.\ebook-output\generative-ai-oss-tutorials.pdf
